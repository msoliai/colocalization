...
options(scipen = 1, digits = 2)
  ...
  gwas.moloc <- gwas[gwas$CHR == as.numeric(bed.data[[14]][1,3]),]
  eqtl.moloc <- eqtl[eqtl$CHR == as.numeric(bed.data[[14]][1,3]),]
  meqtl.moloc <- meqtl[meqtl$CHR == as.numeric(bed.data[[14]][1,3]),]
  data.all <- list(gwas.moloc, eqtl.moloc, meqtl.moloc)
  tmp <- coloc.genome(data.all, bed.data[[14]], prefix = "pref", save.SNP.info=FALSE, cores=2, have_alleles=FALSE, bychrpos=TRUE, prior_var="default", priors=c(1e-04, 1e-06, 1e-07), min_nsnps = 10, takelog = TRUE, write=FALSE, outfolder = "test", forcePVAL = FALSE)

	setwd('/scratch/msoliai/moloc/moloc.output/tagc')
	write.table(tmp, 'vehrv.moloc.asthma.tagc.14.txt', sep='\t', quote=FALSE, row.names=FALSE)
